...
async function showFav(posts: HTMLElement) {
  ...
  if (books) {
    books.forEach((book) => {
      const innerHtml = innerHtmlCodeFav(book);
...
async function addFavListener(book: Book) {
    ...
    if (books) {
      books.forEach((BookElem) => {
        if (BookElem.id === book.id) {
          favBtn.innerHTML = `<img src="https://icons-for-free.com/download-icon-star-131965017658042934_256.ico" class="favBtn">`
          return;
        }
      });
    }
  ...
  favBtn.addEventListener("click", () => {
    ...
    if (heartBtn.className.length > 6) {
      ...
      favBtn.innerHTML = `<img src="https://icons-for-free.com/download-icon-star-131965017658042934_256.ico" class="favBtn hoverFavBtn">`;
...
function innerHtmlCode(book: Book) {
  return `<article class="article">
                <div class="title">
                    ...
                    <div id="rightPart">
                        <div class="artNavbar"><div class="favBtnHolder"><img src="https://icons-for-free.com/download-icon-star-131965017658042934_256.ico" class="favBtn hoverFavBtn"></div></div>
...
function innerHtmlCodeFav(book: Book) {
  return `<article class="article">
                <div class="title">
                    <h3>${book.shortTitle}</h3>
                    <div id="rightPart">
                        <div class="artNavbar"><div class="favBtnHolder"><img src="https://icons-for-free.com/download-icon-star-131965017658042934_256.ico" class="favBtn"></div></div>
                        <h4 class="author">${book.authors}</h4>
                    </div>
                </div>
